...
| GPU runtime | ROCm 7.2.1 (gfx1150) | カーネル実行 |
...
MIGraphX は最初の `session.run()` で **AOT コンパイル** を行い、gfx1150 用の
...
2. .venv を activate, HSA_OVERRIDE_GFX_VERSION=11.5.0 を export
3. config.yaml から PORT を読む (yaml.safe_load via venv の python)
4. nohup python app.py > depth_app.log 2>&1 &
5. /stats を 3 秒間隔で curl し、fps > 0 になるまで最長 180 秒待つ
...
- `HSA_OVERRIDE_GFX_VERSION=11.5.0` が export されているか (`start_all.sh`
  経由なら自動)
...
`config.yaml` の `camera.device` を `/dev/v4l/by-id/...` に変えれば
同じカメラなら追従します。
...
- **FP16 化**: `onnxconverter_common.float16` で半精度に変換、
  メモリ帯域削減効果あり (gfx1150 で約 1.3-1.5x の高速化が期待値)。
